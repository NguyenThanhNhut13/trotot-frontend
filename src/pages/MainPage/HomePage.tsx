...
import { Form, Button, Row, Col, Dropdown, Container } from "react-bootstrap"
import RoomList from "../../components/slider/RoomList"
import HotListings from "../ProductList/HotListings"
import SuggestedRooms from "../ProductList/SuggestedRooms"
...
const HomePage = () => {
  ...
  const apartmentListings = useAppSelector((state) => selectListingsByType(state, "APARTMENT"))

  const apartmentLastFetched = useAppSelector((state) => selectLastFetched(state, "APARTMENT"))
  ...
  useEffect(() => {
    ...
    const isDataFresh = apartmentLastFetched && Date.now() - apartmentLastFetched < 5 * 60 * 1000 // 5 minutes
  ...
  const getPriceRangeParam = () => {
    ...
    if (minPriceInput || maxPriceInput) {
      return {
        minPrice: minPriceInput ? Number.parseFloat(minPriceInput) * 1000000 : undefined,
        maxPrice: maxPriceInput ? Number.parseFloat(maxPriceInput) * 1000000 : undefined,
      }
    }
  ...
  return (
    <div>
                    ...
                    <div className="mb-2">
                      <Dropdown className="w-100">
                        <Dropdown.Toggle className="bg-white text-secondary border-0 w-100 text-start d-flex align-items-center justify-content-between">
                          ...
                          <span className="text-start w-100">{areaLabelMap[areaRange] || "Diện tích"}</span>
                        </Dropdown.Toggle>
                        ...
                        <Dropdown.Menu className="w-100">
                          <div className="p-3">
                            ...
                            <div className="d-flex justify-content-end mt-3">
                              <Button variant="outline-secondary" size="sm" className="me-2" onClick={resetAreaFilters}>
                                Đặt lại
                              </Button>
                              <Button
                                variant="primary"
                    ...
                    <div className="flex-grow-1 px-1" style={{ maxWidth: "270px" }}>
                      <Dropdown className="h-100">
                        <Dropdown.Toggle className="bg-white text-secondary border-0 w-100 h-100 text-start d-flex align-items-center">
                          ...
                          <span className="text-start w-100 ms-2">{areaLabelMap[areaRange] || "Diện tích"}</span>
                        </Dropdown.Toggle>
      ...
      {/* Suggested Rooms - Only show for authenticated users */}
      <SuggestedRooms onSaveRoom={handleSaveRoom} />
